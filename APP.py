...
# ------------------------
# PASO 5: Generación de documentos PDF
# ------------------------
from fpdf import FPDF

st.markdown("---")
st.header("⑤ Generación de documentos en PDF (para adjuntar en correos)")
...
st.write(
    "Aquí generamos una versión PDF de cada documento usando la misma plantilla y los mismos datos. "
    "Estos PDFs son los que después se adjuntarán en los correos."
)

# Usamos la misma regla de nombre, pero con extensión .pdf
regla_nombre_pdf = st.session_state.regla_nombre_archivo

st.markdown("### 🏷️ Nombre de archivo para los PDF")
st.caption(
    "Usamos la misma regla que para los .docx, pero con extensión .pdf. "
    "Ejemplo: Memorial_{{RADICADO}}_{{DEMANDADO}}.pdf"
)

# Mostramos la regla que se usará (solo lectura para simplificar)
st.code(regla_nombre_pdf.replace(".docx", ".pdf"), language="text")

# Detectar placeholders dentro del nombre
placeholders_nombre_pdf = re.findall(r"{{\s*([^}]+?)\s*}}", regla_nombre_pdf)

if placeholders_nombre_pdf:
    st.write("Variables detectadas en el nombre del PDF:")
    st.write(placeholders_nombre_pdf)
else:
    st.info(
        "No se detectaron variables {{...}} en el nombre. "
        "Se usará el mismo nombre para todos los archivos con un consecutivo."
...
if st.button("▶️ Generar documentos PDF"):
    buffer_zip_pdf = BytesIO()
    zf_pdf = zipfile.ZipFile(buffer_zip_pdf, "w", zipfile.ZIP_DEFLATED)

    resultados_pdf = []
    progreso_pdf = st.progress(0)
    total = len(df)

    for idx, (_, fila) in enumerate(df.iterrows(), start=1):
        # 1) Construimos el contexto para docxtpl: placeholder -> valor
        contexto = {}
        for ph, col in mapeo.items():
            if col in df.columns:
                valor = fila[col]
        ...
        # 2) Renderizamos la plantilla a DOCX en memoria
        doc_templ = DocxTemplate(BytesIO(plantilla_bytes))
        doc_templ.render(contexto)

        doc_buffer = BytesIO()
        doc_templ.save(doc_buffer)
        doc_buffer.seek(0)

        # 3) Leemos ese DOCX y extraemos el texto (párrafo por párrafo)
        doc_renderizado = Document(doc_buffer)
        texto_parrafos = [p.text for p in doc_renderizado.paragraphs]

        # 4) Creamos un PDF simple con ese texto
        pdf = FPDF()
        pdf.set_auto_page_break(auto=True, margin=15)
        pdf.add_page()
        pdf.set_font("Helvetica", size=11)

        for p_text in texto_parrafos:
            # Evitamos problemas con None
            if p_text is None:
                p_text = ""
            # MultiCell para manejar saltos de línea automáticos
        ...
        # 5) Construimos el nombre del PDF a partir de la regla
        nombre_pdf = regla_nombre_pdf

        for ph in placeholders_nombre_pdf:
            # Elegimos la columna: del mapeo o del mismo nombre
            if ph in mapeo and mapeo[ph] in df.columns:
                col = mapeo[ph]
            elif ph in df.columns:
                col = ph
            ...
            if col is not None:
                ...
                if pd.isna(valor):
                    valor = ""
                patron = r"{{\s*" + re.escape(ph) + r"\s*}}"
                nombre_pdf = re.sub(patron, str(valor), nombre_pdf)
            else:
                patron = r"{{\s*" + re.escape(ph) + r"\s*}}"
                nombre_pdf = re.sub(patron, "", nombre_pdf)

        # Nos aseguramos de que termine en .pdf
        nombre_pdf = nombre_pdf.replace(".docx", "").replace(".DOCX", "")
        if not nombre_pdf.lower().endswith(".pdf"):
            nombre_pdf = nombre_pdf + ".pdf"

        if nombre_pdf.strip() == ".pdf" or nombre_pdf.strip() == "":
            nombre_pdf = f"documento_{idx}.pdf"

        # 6) Guardamos el PDF en el ZIP
        zf_pdf.writestr(nombre_pdf, pdf_buffer.read())
        ...
        progreso_pdf.progress(idx / total)

    zf_pdf.close()
    buffer_zip_pdf.seek(0)

    st.success(f"Se generaron {len(resultados_pdf)} documentos PDF.")

    st.markdown("### Ejemplo de PDFs generados:")
    st.dataframe(pd.DataFrame(resultados_pdf).head(10))

    st.download_button(
        label="⬇️ Descargar todos los PDFs (.zip)",
        data=buffer_zip_pdf.getvalue(),
        file_name="documentos_generados_pdf.zip",
        mime="application/zip"
    )
